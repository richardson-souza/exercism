...
class MarkdownTest(unittest.TestCase):
    ...
    def test_parsing_headers_level(self):
        input_data = [f"{'#'*i} This will be an h{i}" for i in range(1,7)]
        result_data = [f'<h{i}>This will be an h{i}</h{i}>' for i in range(1,7)]
        for i in range(len(input_data)):
            self.assertEqual(parsing_headers_level(input_data[i]), result_data[i], msg=f'Expected {result_data[i]} header level.')
