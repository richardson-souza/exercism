...
def match_paragraph(text: str) -> Union[re.Match, None]:
    """Identify if text has a paragraph.

    :param text: str - Any text.
    :return: re.Match or None.
    """
    return re.match(STR_PARAGRAPH, text)
...
def parsing_bold_italic(text: str) -> str:
    """Convert text with Markdown code for bold or talicize in a valid HTML <strong>.

    :param text: str - Any text.
    :return: str - Text with strong importance element and/or  emphasis element.
    """
...
def parsing_list(text: str, in_list: bool) -> Union[str, bool]:
    ...
    """
    if match_bold_text(text) is not None or match_italic_text(text) is not None:
...
def parsing_paragraph(text: str) -> str:
    """Add valid HTML <p>.

    :param text: str - Literal with valid Mardown code.
    :return: str - Text with paragraph element <p>paragraph</p>.
    """
    if not match_paragraph(text):
        return '<p>' + text + '</p>'
    return text
...
def parse(markdown: str) -> str:
    ...
    for i in markdown.split('\n'):
        ...
        if match:
            i, in_list = parsing_list(match.group(1), in_list)
        else:
            if in_list:
                in_list_append = True
                in_list = False

        i = parsing_paragraph(i)
